...
public class RouteReader {
	...
	public static boolean recvRoute(String key, String origin, String dest, final RouteListener listener){
		String url = null;
		try {
			...
			url = String.format(
				...
				"origin=%s&destination=%s&mode=driving&sensor=false&key=%s",origin2,dest2,key);
	...
	public static boolean recvPlace(String apiKey, String type, LatLng loc, int radius, final PlaceListener listener){
		...
		try {
			...
			url = String.format(
				"https://maps.googleapis.com/maps/api/place/search/json?language=ja&key=%s&types=%s&location=%f,%f&radius=%d",
